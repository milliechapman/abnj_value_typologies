...
```{r}
r_nspp_all_as <- rescale_raster(r_nspp_all)# "bio_nspp"
...
```{r}
r_rls_all_as <- rescale_raster(r_rls_all)
...
```{r}
plot(s_phys_seamounts[[3]], col = cols, main="s_phys_seamounts")
```
...
p01_features <- stack(
  r_nspp_all_as,
  r_rls_all_as,
  rescale_raster(s_phys_seamounts[[3]]),
...
```{r}
# create problem
p10 <- problem(r_pu_areas, features = p01_features) %>%
...
p30 <- problem(r_pu_areas, features = p01_features) %>%
      add_min_set_objective() %>%
...
p01 <- problem(r_pu_areas, p01_features) %>%
   ...
   add_max_utility_objective(budget = 0.01) %>% # 10% of total high seas area
    #add_relative_targets(c(0.3, 0.3, 0)) %>%
    add_default_solver(gap = 0) 
